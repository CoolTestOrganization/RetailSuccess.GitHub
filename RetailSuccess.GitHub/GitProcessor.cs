...
namespace RetailSuccess.GitHub
{
    public class GitProcessor
    {
        ...
        {
            await _commandProcessor.SendAsync(new CreateRepository.Command(repositoryName,
                _gitHubClientOptions.OrganizationName));
            var repositoryQuery =
                await _queryProcessor.ExecuteAsync(new GetRepositoryByName.Query(_gitHubClientOptions.OrganizationName,
                    repositoryName));
            ...
            await _commandProcessor.SendAsync(new AddRepositoryToTeam.Command(repositoryName,
                _gitHubClientOptions.OrganizationName, _gitHubClientOptions.DefaultTeamName));
